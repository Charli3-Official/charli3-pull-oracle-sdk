...
wait_for_port() {
  ...
  while true; do
    # Check localhost (standard)
    if (echo > /dev/tcp/localhost/$port) >/dev/null 2>&1; then
      echo "$name is up and running on port $port (localhost)!"
      set -x
      break
    fi

    # Check 127.0.0.1 (IPv4 explicit)
    if (echo > /dev/tcp/127.0.0.1/$port) >/dev/null 2>&1; then
      echo "$name is up and running on port $port (127.0.0.1)!"
      set -x
      break
    fi

    # Fallback: Try curl on localhost
    if command -v curl >/dev/null 2>&1; then
      if curl -s -o /dev/null "http://localhost:$port"; then
        echo "$name is up and running on port $port (curl localhost)!"
        set -x
        break
      fi
    fi
